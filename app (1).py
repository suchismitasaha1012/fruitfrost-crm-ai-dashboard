...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with col_a:
        ...
        fig_lmr = px.scatter(dff, x="monthly_revenue_inr", y="ltv_predicted_inr",
                              color="health_tier", color_discrete_map=COLOR_RISK,
                              hover_name="outlet_name",
                              labels={"monthly_revenue_inr":"Monthly Revenue (₹)","ltv_predicted_inr":"Predicted 24M LTV (₹)"})
        # Manual trendline (no statsmodels needed)
        z = np.polyfit(dff["monthly_revenue_inr"], dff["ltv_predicted_inr"], 1)
        p = np.poly1d(z)
        x_line = np.linspace(dff["monthly_revenue_inr"].min(), dff["monthly_revenue_inr"].max(), 100)
        fig_lmr.add_traces(go.Scatter(x=x_line, y=p(x_line), mode="lines",
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if submitted:
        ...
        rpi_new   = round(churn_prob_new * (ltv_pred_new/df["ltv_predicted_inr"].max()) * 0.46 * iv_eff.get(iv_new,0) * 100, 2)
